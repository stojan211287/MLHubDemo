...
class FeatureDef:
    ...
    def parse(self, data):
        ...
        if isinstance(data, pd.DataFrame):
            ...
            raise ValueError(
                "Variable data can be either a Pandas Dataframe or a dictionary!"
            )
        ...
        try:
            ...
            if self.recipe is None:
                raise ValueError(
                    "You most provide a recipe for feature transformation!"
                )
        ...
        return self.wrapper(
            name=self.name, kind=self.kind, values=self._ensure_type(feature_values)
        )
...
class FeatureParser:
    def __init__(self, features):
        ...
        for feature in features:
            try:
                ...
                raise ValueError(
                    "You most provide a list of FeatureDefs to FeatureParser!"
                )
    ...
    def add_parsed_to_data(self, data):
        ...
        return pd.concat([custom_features_df, data.loc[:, list(raw_features)]], axis=1)
        ...
        for feature in self.features:
            ...
            if feature.kind == "numeric":
                feature_columns.append(
                    tf.feature_column.numeric_column(key=feature_key, dtype=tf.float32)
                )
            elif feature.kind == "categorical":
                feature_vocab = np.unique(feature.parse(data))
                feature_columns.append(
                    tf.feature_column.categorical_column_with_vocabulary_list(
                        key=feature_key, dtype=tf.string, vocabulary_list=feature_vocab
                    )
                )
            else:
                raise ValueError(
                    "Feature kinds can only be 'numeric' or 'categorical', not %s"
                    % (feature.kind,)
                )
...
if __name__ == "__main__":
    ...
    features = [
        FeatureDef(
            name="log1pOf1",
            kind="numeric",
            recipe={"generators": [1], "function": np.log1p},
        ),
        FeatureDef(
            name="sumOfLog1pOf1And2",
            kind="numeric",
            recipe={
                "generators": [1, 2],
        ...
        FeatureDef(name=3, kind="numeric"),
        FeatureDef(
            name="3rdFeature",
            kind="numeric",
            recipe={"generators": [3], "function": lambda x: np.array(x)},
        ),
    ]
