...
class KerasModel:
    ...
    def _parse_model_code(self, d_in, d_out):
        ...
        try:
            model_code = KerasModel._complete_model_code(
                user_code=self.code, d_in=d_in, d_out=d_out
            )

            print(model_code)

            model_code_module = exec_user_code(
                code=model_code, module_name="custom_keras_model_%s" % (model_code,)
            )
            ...
            raise UserCodeExecutionError(
                "There was an error in your model code: %s" % (str(error),)
            )
    ...
    def train_and_eval(self, features, target_name, epochs=100, learning_rate=1e-2):
        ...
        try:
            ...
            train_set, train_target, test_set, test_target = KerasModel._train_test_split(
                features=features, target_name=target_name
            )

            train_set_np = KerasModel._convert_features(features=train_set)
            train_target_np = KerasModel._convert_target(
                target=train_target, num_classes=num_classes
            )

            test_set_np = KerasModel._convert_features(features=test_set)
            test_target_np = KerasModel._convert_target(
                target=test_target, num_classes=num_classes
            )
            ...
            model = self._parse_model_code(
                d_in=train_set_np.shape[1], d_out=num_classes
            )
            ...
            model.fit(
                train_set_np, train_target_np, epochs=epochs, batch_size=train_batch
            )
            ...
            raise InputDataError(
                "Error while converting input data: %s" % (str(error),)
            )

        except RuntimeError as error:
            raise UserCodeExecutionError(
                "Error while training model: %s" % (str(error),)
            )
    ...
    def deploy(self, model_id):
        ...
        if self.model_instance is not None:
            ...
            self.model_instance.save(
                os.path.join(save_dir, "model_%s.hdf5" % (model_id,))
            )
    ...
    def _train_test_split(
        features: pd.DataFrame, target_name: str, test_set_size: float = 0.1
    ):

        train_indices = np.random.choice(
            features.index.values, size=int((1.0 - test_set_size) * features.shape[0])
        )

        test_indices = np.setdiff1d(
            features.index.values, train_indices, assume_unique=True
        )
